      ...
      devShells = forEachSupportedSystem (
        { pkgs }:
        {
          default = pkgs.mkShell {
            ...
            shellHook = ''
              # SDL2 + Gentoo system libraries (this is the key)
              ...
              # Use Gentoo's native Vulkan loader + layers (exactly like outside the shell)
              export VK_LAYER_PATH="/usr/share/vulkan/explicit_layer.d"

              # Point to Gentoo's ICD files (your RADV driver)
              ...
              echo "   LD_LIBRARY_PATH now includes /usr/lib64 + LLVM slot"
            '';
          };
        }
      );
